					...
					<table class="table table-striped table-bordered table-hover" id="dataCalon">
						...
						<tbody>
							...
							while ($hasil = mysqli_fetch_array($calon)) {
								...
								<td align=center> <button type="button" class="btn btn-sm btn-primary" data-toggle="modal" data-target="#editCalon" data-id="<?= $hasil[0] ?>" data-nama="<?= $hasil[1] ?>" data-foto="<?= $hasil[2] ?>"><i class="icon-pencil icon-white"></i> Edit</button>
...
if ($_POST['editcalon']) {
	...
	move_uploaded_file($lokasi_file, $direktori);
	if (empty($nama_file)) {
		$query = mysqli_query($mysqli, "UPDATE calon_rt set no_urut='$nourut', nama='$nama' where no_urut='$id' ") or die(mysqli_error($mysqli));
	} else {
		$query = mysqli_query($mysqli, "UPDATE calon_rt set no_urut='$nourut', nama='$nama', foto='$nama_file' where no_urut='$id' ") or die(mysqli_error($mysqli));
	}
	if ($query) {
