...
$query = mysqli_query($mysqli, "SELECT * from login where username='$user'") or die(mysqli_error($mysqli));
...
if ($_POST['update']) {
	...
	do {
		if ($pasl == $passm) {
			if (($userbaru != "") && ($pasbaru != "")) {
				$query = mysqli_query($mysqli, "UPDATE login set username='$userbaru', password='$pasbarum' where username='$user' and password='$passm'") or die(mysqli_error($mysqli));
				$_SESSION['username'] = $userbaru;
				echo "<meta http-equiv=refresh content=0;url=?hl=aturuser&info=1>";
				break;
			} else if (($pasbaru == "") && ($userbaru != "")) {
				$query = mysqli_query($mysqli, "UPDATE login set username='$userbaru' where username='$user' and password='$passm'") or die(mysqli_error($mysqli));
				$_SESSION['username'] = $userbaru;
				echo "<meta http-equiv=refresh content=0;url=?hl=aturuser&info=2>";
				break;
			} else if (($pasbaru != "") && ($userbaru == "")) {
				$query = mysqli_query($mysqli, "UPDATE login set password='$pasbarum' where username='$user' and password='$passm'") or die(mysqli_error($mysqli));
